 ...
 * @returns {number}
 */
function KDGenericMultEnchantmentAmount(amt, item, Loot, curse) {
...
/** @type {Record<string, {level: number, weight: (item: string) => number, events: (item: string, Loot: any, curse: string) => KinkyDungeonEvent[]}>} */
let KDEventEnchantmentModular = {
	"Evasion": {level: 1,
		...
		events: (item, Loot, curse) => {
			...
			amt = KDGenericMultEnchantmentAmount(amt, item, Loot, curse);
			return [
	...
	"ManaCost": {level: 5,
		...
		events: (item, Loot, curse) => {
			let power = Math.max(KinkyDungeonGetRestraintByName(item)?.power || 0, 1);
	...
	"ManaRegen": {level: 2,
		...
		events: (item, Loot, curse) => {
			let power = Math.max(KinkyDungeonGetRestraintByName(item)?.power || 0, 3);
			let amt = 15 + Math.round(KDRandom() * 10 * power);
			amt = KDGenericMultEnchantmentAmount(amt, item, Loot, curse);
			return [
