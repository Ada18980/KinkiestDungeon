...
function KDDrawEnemyTooltip(enemy, offset) {
	...
	if (enemy.slow || KDEntityBuffedStat(enemy, "MoveSpeed") < 0) statuses.push({name: "Slow", count: enemy.slow});
...
function KinkyDungeonEnemyLoop(enemy, player, delta, visionMod, playerItems) {
	...
	AIData.wantsToAttack = (
		!AIData.ignore
		&& (enemy == KinkyDungeonLeashingEnemy() || !KDGameData.KinkyDungeonLeashedPlayer || KinkyDungeonFlags.has("PlayerCombat"))
	...
	if (!AIData.aggressive && player.player && (enemy.playWithPlayer || KDAllied(enemy))) {
		...
		AIData.followRange = (enemy.usingSpecial ? enemy.Enemy.specialRange : undefined) || enemy.Enemy.attackRange || AIData.followRange;
	}
