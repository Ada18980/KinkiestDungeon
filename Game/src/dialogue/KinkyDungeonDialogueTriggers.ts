...
let KDDialogueTriggers: Record<string, KinkyDialogueTrigger> = {
	...
	"OfferChastity": {
		...
		weight: (_enemy, _dist) => {
			return .1 + 0.1 * Math.max(Math.abs(KinkyDungeonGoddessRep.Metal)/100, Math.abs(KinkyDungeonGoddessRep.Elements)/100, Math.abs(KinkyDungeonGoddessRep.Illusion)/100, Math.abs(KinkyDungeonGoddessRep.Ghost)/100);
		},
	},
	"OfferCursed": {
		...
		weight: (_enemy, _dist) => {
			return 1 + 0.8 * Math.max(Math.abs(KinkyDungeonGoddessRep.Leather)/100, Math.abs(KinkyDungeonGoddessRep.Ghost)/100);
		},
	},
