...
let KDInventoryActionPerRow = 9;
let KDDefaultYOffForInventoryContainer = -94; // 224
...
/**
 * Returns list of restraints, without hosts
 */
		...
		for (let item of values) {
			...
			if (preview
				...
				&& (!namefilter
					...
					|| (item.type == Weapon && KDWeapon(item).tags?.some((tag) => {
						return tag.toLocaleLowerCase().includes(namefilter.toLocaleLowerCase())
					}))|| (item.type == Consumable && KDConsumable(item).tags?.some((tag) => {
						return tag.toLocaleLowerCase().includes(namefilter.toLocaleLowerCase())
					}))
...
)
{
	if (!noscroll) {
		if (KDToggles.SpellBook) {
			...
			KDDraw(kdcanvas, kdpixisprites, "magicBook",
				KinkyDungeonRootDirectory + "MagicBookNew.png", 
				xOffset + canvasOffsetX_ui, 
				yOffset + canvasOffsetY_ui, 640*KinkyDungeonBookScale, 520*KinkyDungeonBookScale, undefined, {
					zIndex: 128,
				});
		} else {
			...
			FillRectKD(kdcanvas, kdpixisprites, "magicBook", {
				...
				Top: canvasOffsetY_ui + yOffset + 90,
	...
	DrawTextFitKD(nameText, 
		xOffset + canvasOffsetX_ui + 640*KinkyDungeonBookScale/3.35, 
		yOffset + canvasOffsetY_ui + 483*KinkyDungeonBookScale/5, 300, KDBookText, KDTextTan, undefined, undefined, 129);
	...
	if (showpreview) {
		...
		KDDraw(kdcanvas, kdpixisprites, "preview",
			item.preview, 
			xOffset + canvasOffsetX_ui + 640*KinkyDungeonBookScale/3.35 - 50, 
			yOffset + canvasOffsetY_ui + 483*KinkyDungeonBookScale/5 + 40, 100, 100, undefined, {
				zIndex: 129,
		...
		if (item.item.type == Restraint) {
			...
			if (lock) {
				KDDraw(kdcanvas, kdpixisprites, "preview_lock",
					KinkyDungeonRootDirectory + "Locks/" + lock + ".png", 
					xOffset + canvasOffsetX_ui + 640*KinkyDungeonBookScale/3.35 - size/2 + 70, 
					yOffset + canvasOffsetY_ui + 483*KinkyDungeonBookScale/5 + 40 + (100 - size)/2, 56, 56, undefined, {
						zIndex: 129,
					}, undefined, undefined, undefined, true);
			}
		}
		...
		if (item.item.type == Restraint || item.item.type == LooseRestraint) {
			...
			if (goddesses)
				DrawTextFitKD(TextGet("KDGoddess") + goddesses, 
			xOffset + canvasOffsetX_ui + 640*KinkyDungeonBookScale/3.35, 
			yOffset + canvasOffsetY_ui + 483*KinkyDungeonBookScale/5 + 435, 300, KDBookText, KDTextTan, 22, undefined, 130);
			...
			if (maxSuff) {
				DrawTextKD(
					maxSuff, 
					75 + xOffset + canvasOffsetX_ui + 640*KinkyDungeonBookScale/3.35, 
					yOffset + 2 + canvasOffsetY_ui + 483*KinkyDungeonBookScale/5 + 375, KDBookText, KDTextTan, 28, undefined, 130);
			}
			DrawTextKD(
				TextGet("KinkyDungeonConsumableQuantity") + item.item.quantity, 
				(!!maxSuff ? -75 : 0) + xOffset + canvasOffsetX_ui + 640*KinkyDungeonBookScale/3.35, 
				yOffset + canvasOffsetY_ui + 483*KinkyDungeonBookScale/5 + 375, KDBookText, KDTextTan, 30, undefined, 130);
			DrawTextKD(TextGet("KinkyDungeonRarity") + TextGet("KinkyDungeonRarity" + consumable.rarity),
			 xOffset + canvasOffsetX_ui + 640*KinkyDungeonBookScale/3.35, 
			 yOffset + canvasOffsetY_ui + 483*KinkyDungeonBookScale/5 + 410, KDBookText, KDTextTan, 22, undefined, 130);
			...
			DrawTextFitKD(st, 
				xOffset + canvasOffsetX_ui + 640*KinkyDungeonBookScale/3.35, 
				yOffset + canvasOffsetY_ui + 483*KinkyDungeonBookScale/5 + 24, 300, KDBookText, KDTextTan, 18, undefined, 129);
			...
			for (let tagi = -1; tagi < tags.length; tagi++) {
				if (tagi == -1) {
					...
					KDDraw(kdcanvas, kdpixisprites, "wt_" + tagi,
						...
						yOffset + canvasOffsetY_ui + 483*KinkyDungeonBookScale/5 + 46 - spritesize + 133,
					...
					if (MouseIn(
						...
						mult = KDGetFontMult();
						textSplit = KinkyDungeonWordWrap(TextGet("KDWeaponTagD_damage"), 13*1.3*mult, 30*1.3*mult).split('\n');
					}
				} else {
					// Draw the tag
					KDDraw(kdcanvas, kdpixisprites, "wt_" + tagi,
			...
			DrawTextKD(TextGet("KinkyDungeonWeaponDamage") + Math.round(weapon.damage * 10), 
			xOffset - off + canvasOffsetX_ui + 640*KinkyDungeonBookScale/3.35, 
			yOffset + canvasOffsetY_ui + 483*KinkyDungeonBookScale/5 + 350, KDBookText, KDTextTan, 24, undefined, 130);
			if (off) DrawTextKD(TextGet("KinkyDungeonWeaponDamageBind") + (bind ? Math.round(bind * 10) : (bindEff ? Math.round(bindEff * 100) + "%" : "")), 
			xOffset + off + canvasOffsetX_ui + 640*KinkyDungeonBookScale/3.35, 
			yOffset + canvasOffsetY_ui + 483*KinkyDungeonBookScale/5 + 350, KDBookText, KDTextTan, 24, undefined, 130);

			if (!weapon.nocrit) {
				DrawTextKD(TextGet("KinkyDungeonWeaponCrit") + Math.round((weapon.crit || KDDefaultCrit) * 100) + "%", 
				xOffset - off + canvasOffsetX_ui + 640*KinkyDungeonBookScale/3.35, 
				yOffset + canvasOffsetY_ui + 483*KinkyDungeonBookScale/5 + 380, KDBookText, KDTextTan, 24, undefined, 130);
				if (off) DrawTextKD(TextGet("KinkyDungeonWeaponBindCrit") + Math.round((weapon.bindcrit || KDDefaultBindCrit) * 100) + "%", 
				xOffset + off + canvasOffsetX_ui + 640*KinkyDungeonBookScale/3.35, 
				yOffset + canvasOffsetY_ui + 483*KinkyDungeonBookScale/5 + 380, KDBookText, KDTextTan, 24, undefined, 130);
			} else {
				DrawTextKD(TextGet("KinkyDungeonWeaponNoCrit"), 
				xOffset + canvasOffsetX_ui + 640*KinkyDungeonBookScale/3.35, 
				yOffset + canvasOffsetY_ui + 483*KinkyDungeonBookScale/5 + 380, KDBookText, KDTextTan, 24, undefined, 130);
			...
			yOffset + canvasOffsetY_ui + 483*KinkyDungeonBookScale/5 + 440, KDBookText, KDTextTan, 24, undefined, 130);
		...
		for (let N = 0; N < textSplit.length; N++) {
			DrawTextFitKD(textSplit[N],
				xOffset + canvasOffsetX_ui + 640*KinkyDungeonBookScale/3.35, 
				yOffset + canvasOffsetY_ui + 483*KinkyDungeonBookScale/5  + 155 + i * 23, 330, KDBookText, KDTextTan, 20, undefined, 130); i++;}
	...
	for (let N = 0; N < data.extraLinesPre.length; N++) {
		DrawTextFitKD(data.extraLinesPre[N],
			...
			yOffset + canvasOffsetY_ui + 483*KinkyDungeonBookScale/5 + i * 32, 640*KinkyDungeonBookScale/2.5, data.extraLineColorPre[N], data.extraLineColorBGPre[N], 20, undefined, 130); i++;}
	for (let N = 0; N < textSplit2.length; N++) {
		DrawTextFitKD(textSplit2[N],
			xOffset + canvasOffsetX_ui + 640*KinkyDungeonBookScale*(1-1.0/3.35), 
			yOffset + canvasOffsetY_ui + 483*KinkyDungeonBookScale/5 + i * 32, 640*KinkyDungeonBookScale/2.5, KDBookText, KDTextTan, 20, undefined, 130); i++;}
	if ((unidentified && item.item.type == Restraint) || (!unidentified)) {
		for (let N = 0; N < data.extraLines.length; N++) {
			DrawTextFitKD(data.extraLines[N],
				xOffset + canvasOffsetX_ui + 640*KinkyDungeonBookScale*(1-1.0/3.35), 
				yOffset + canvasOffsetY_ui + 483*KinkyDungeonBookScale/5 + i * 32, 640*KinkyDungeonBookScale/2.5, data.extraLineColor[N], data.extraLineColorBG[N], 20, undefined, 130); i++;}
	}
	i = 0;
	...
	if (filteredInventory) {
		...
		if (!KDRenderAlternateInventory(selected, xOffset, yOffset, prefix)) {
			...
			if (!nosearch) {
				/*DrawTextFitKD(
					,
					1460 + xOffset + 350/2, yOffset + 150, 200, KDBaseWhite, KDTextGray0, 18, "center");*/
				let TF = KDTextField(prefix + "InvFilter", 1460 + xOffset, yOffset + 480*KinkyDungeonBookScale + 100, 
					350, 54, "text", "", "45");
				if (TF.Created) {
					KDInvFilter = "";
			...
			let drawn: ProgressListData = KDDrawScrollableList(listID, true, (
				...
				list: KDScrollableListData)  => {
					...
					for (let i = 0; i < numRows; i++) {
						...
						if (yy >= maxList) break;
						let index = numRows * listRow + i;//(prefix ? KinkyDungeonContainerOffset : KinkyDungeonInventoryOffset);
						if (filteredInventory[index] && filteredInventory[index].item) {
							...
							let hk = KDFindHotkeyInGrid(index, currentPage, filteredInventory, numRows, 
							...
							*/
							if (DrawButtonKDExTo(container, prefix + "invchoice_" + index, (bdata) => {
								let diff = false;
								if (prefix) {
								...
								if (itemcallback) itemcallback(filteredInventory[index],
									...
									yOffset + KDBaseInventoryOffset + KDDefaultYOffForInventoryContainer + b_height * yy,
							...
							list.x + xx * b_width, 
							list.y + b_height * (yy + visualIndex),
							...
							if (KDGameData.InventoryAction && KDInventoryAction[KDGameData.InventoryAction]?.itemlabel
								...
								DrawTextFitKD(KDInventoryAction[KDGameData.InventoryAction].itemlabel(KinkyDungeonPlayerEntity, filteredInventory[index].item),
									36 + list.x + xx * b_width,
									list.y + b_height * (yy + visualIndex) + (useIcons ? 72 : 36) - 9,  72, KDInventoryAction[KDGameData.InventoryAction].itemlabelcolor ? KDInventoryAction[KDGameData.InventoryAction].itemlabelcolor(KinkyDungeonPlayerEntity, filteredInventory[index].item) : KDBaseWhite,
									KDTextGray0, 18, "center");
							}
						} else {
							// no longer necessary with scrollable list
							...
							DrawButtonKDExTo(container, prefix + "invchoice_" + index, (_bdata) => {
								//KinkyDungeonCurrentPageInventory = index;
								return true;
							}, true, 
							list.x + xx * b_width, 
							...
							/*FillRectKD(kdcanvas, kdpixisprites, "kdInvEmptySpot" + i, {
								...
								Top: KDBaseInventoryOffset + KDDefaultYOffForInventoryContainer + b_height * yy,
			...
			canvasOffsetX_ui + xOffset + 640*KinkyDungeonBookScale + 526, 
			yOffset + KDBaseInventoryOffset + KDDefaultYOffForInventoryContainer - 50, 90, 44, "", KinkyDungeonInventoryOffset > 0 ? KDBaseWhite : "#888888", KinkyDungeonRootDirectory + "Up.png",
			...
			canvasOffsetX_ui + xOffset + 640*KinkyDungeonBookScale + 526, 
			yOffset + 480*KinkyDungeonBookScale + 100, 90, 44, "", ((prefix ? KinkyDungeonContainerOffset : KinkyDungeonInventoryOffset) + 24 < filteredInventory.length) ? KDBaseWhite : "#888888", KinkyDungeonRootDirectory + "Down.png",
			undefined, undefined, undefined, undefined, undefined, undefined, {
				hotkey: KDHotkeyToText(KDInventoryDrawContainerHotkeys[prefix] ?
		...
		for (let i = 0; i < filters.length - index + 2 * KDMaxFilters + 1; i++) {
			...
			if (index > 0 && i == index) {
				...
				DrawButtonKDExScroll(prefix + "invchoice_filter_" + i, (amount) => {
					...
					return true;
				}, true, canvasOffsetX_ui + xOffset + xx * 200 + 640*KinkyDungeonBookScale + 132, yOffset + KDBaseInventoryOffset + KDDefaultYOffForInventoryContainer + 40 * yy, 159, 36,
...
function KDDrawInventoryFilters(xOffset, yOffset = 0, skipfilters = [], addFilters = [], spacing = 93, perColumn = 6, hotkeyUp = "Default", hotkeyDown = "Default") : string {
	...
	for (let I = 0; I < KDFilters.length; I++) {
		...
		if (!KDAlternateInventoryRender()) {
			let label = TextGet("KinkyDungeonCategoryFilter" + KDFilters[I]);
			...
			yOffset + KDBaseInventoryOffset + KDDefaultYOffForInventoryContainer + YY*spacing, 
			(KinkyDungeonCurrentFilter == KDFilters[I] && perColumn >= 6) ? 110 : 90, 90,
			"",
				(KinkyDungeonCurrentFilter == KDFilters[I]) ? KDBaseWhite : col, 
				KinkyDungeonRootDirectory + "UI/InvFilter/" + KDFilters[I] + ".png", "", undefined, 
...
let KD_invactiontextanchorx_offset = 1400;
let KD_invactiontextanchory = 80;
...
function KinkyDungeonDrawInventory() {
	...
	KDDrawInventoryFilters(xOffset + 515, 0, ffilters); // 475 original
	...
	let ss = KDDrawInventoryContainer(xOffset - 200, 0, filteredInventory, filter, KinkyDungeonCurrentFilter);
	...
	if (KinkyDungeonDrawInventorySelected(filteredInventory[KinkyDungeonCurrentPageInventory],
		undefined, undefined, xOffset, KDDefaultYOffForInventoryInfo) && !KDAlternateInventoryRender()) {
		...
		if (inventoryActions.length > 0) {
			let XX = canvasOffsetX_ui + xOffset + 70;
			let YY = 34 + 483*KinkyDungeonBookScale - 5 + 38;
			let YYTooltip = YY - 30;
			let II = 0;
					...
					if (KDInventoryAction[action]) {
						...
						if (KDInventoryAction[action]?.label
							...
							DrawTextFitKD(KDInventoryAction[action].label(KinkyDungeonPlayerEntity, filteredInventory[KinkyDungeonCurrentPageInventory].item),
								XX + II*KDInventoryActionSpacing + (KDInventoryAction[action]?.doubleSize ? (74 + KDInventoryActionSpacing)/2 : 34), YY + 72 - 9, 72, KDInventoryAction[action].labelcolor ? KDInventoryAction[action].labelcolor(KinkyDungeonPlayerEntity, filteredInventory[KinkyDungeonCurrentPageInventory].item) : KDBaseWhite,
								KDTextGray0, 18, "center");
						if (DrawButtonKDEx("invAction" + action, (_bdata) => {
							...
							return true;
						}, true, XX + II*KDInventoryActionSpacing, YY, 
						KDInventoryAction[action]?.doubleSize ? (74 + KDInventoryActionSpacing) : 74, 74, 
						KDInventoryAction[action]?.doubleSize ? TextGet("KDInventoryAction" + action) : "", 
						KDInventoryAction[action]?.doubleSize ?  (KDInventoryAction[action].valid(KinkyDungeonPlayerEntity, filteredInventory[KinkyDungeonCurrentPageInventory].item) ? (
								KDInventoryAction[action].highlight ? KDInventoryAction[action].highlight(KDPlayer(), filteredInventory[KinkyDungeonCurrentPageInventory].item) : KDBaseWhite
							) : KDBaseRed) : "",
						...
						undefined, KDInventoryAction[action]?.doubleSize, {
	...
	KinkyDungeonDrawMessages(true, 0, false, undefined, 50, 2);
	...
	*/
	if (KinkyDungeonControlsEnabled() && !KDToggleShowAllBuffs) {
		...
		for (let i = 1; i <= KDNumOfQuickLoadouts; i++) {
			...
			if (MouseIn(630, QL_y + 70 * i, 120, 60)) {
				...
				for (let ii = 0; ii < 20 && ii < (KDGameData.QuickLoadouts ? (KDGameData.QuickLoadouts[i+""] ? KDGameData.QuickLoadouts[i+""].length : 0) : 0); ii++) {
					...
					DrawTextKD(str, 780, QL_y + ii * 25, KinkyDungeonInventoryGet(item) ? KDBaseWhite : KDBaseRed, undefined, 22, "left");
				}
			}
		}
		...
		DrawButtonKDEx("quickinvbg2_button", (_bdata) => {
			return true;
		}, true, 510, 50, 255, PIXIHeight - 230, "", KDButtonColor, undefined, undefined, false, true,
...
/** Gets the hotkeys for maneuvering around in a grid */
function KDFindHotkeyInGrid(index: number, currentPage: number, gridList: any[], numColumns: number, hotkeyUp: string, hotkeyDown: string, hotkeyLeft: string, hotkeyRight: string) {
	let hk = "";
	if (index == currentPage + 1
		|| ((currentPage == gridList.length - 1) && index == 0)) hk = hotkeyRight;
	else if (index == currentPage - 1
		|| ((currentPage == 0) && index == gridList.length - 1)) hk = hotkeyLeft;
	else if (index == currentPage + numColumns
		|| ((currentPage + numColumns > gridList.length - 1)
		&& ((gridList[currentPage + numColumns - (Math.ceil(gridList.length/numColumns) * numColumns)]
			&& index == currentPage + numColumns - (Math.ceil(gridList.length/numColumns) * numColumns)
		)
			|| (!gridList[currentPage + numColumns - (Math.ceil(gridList.length/numColumns) * numColumns)]
				&& index == currentPage + numColumns - (Math.floor(gridList.length/numColumns) * numColumns)
			)))) hk = hotkeyDown;
	else if (index == currentPage - numColumns
		|| ((currentPage - numColumns < 0)
		&& ((gridList[currentPage - numColumns + (Math.ceil(gridList.length/numColumns) * numColumns)]
			&& index == currentPage - numColumns + (Math.ceil(gridList.length/numColumns) * numColumns))
			|| (!gridList[currentPage - numColumns + (Math.ceil(gridList.length/numColumns) * numColumns)]
