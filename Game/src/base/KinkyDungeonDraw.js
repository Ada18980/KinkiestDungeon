...
function KinkyDungeonDrawGame() {
	...
	if (KinkyDungeonDrawState == "Game") {
		...
		if ((KinkyDungeonIsPlayer() || (KinkyDungeonGameData && CommonTime() < KinkyDungeonNextDataLastTimeReceived + KinkyDungeonNextDataLastTimeReceivedTimeout))) {
			...
			let lightmapx = kdgameboard.x < 0 ? kdgameboard.x : 0;
			let lightmapy = kdgameboard.y < 0 ? kdgameboard.y : 0;
			if (kdbrightnessmapGFX.x != lightmapx
				|| kdbrightnessmapGFX.y != lightmapy) {
				kdbrightnessmapGFX.x = lightmapx;
				kdbrightnessmapGFX.y = lightmapy;
				kdlightmapGFX.x = lightmapx;
				kdlightmapGFX.y = lightmapy;
				KDRenderLightmaps();
			}
	...
	if (!KinkyDungeonMessageToggle || NoLog) {
		...
		if (!MouseIn(KDMsgX + (KDMsgWidth - KDMsgWidthMin)/2, 0, KDMsgWidthMin, 62 + KDLogDist*(2 + KDMaxConsoleMsg)) || KinkyDungeonDrawState != "Game") {
			...
			if (KinkyDungeonActionMessageTime > 0 && KinkyDungeonActionMessageNoPush) {
				DrawTextFitKD(KinkyDungeonActionMessage, KDMsgX + KDMsgWidth/2, 15 + spacing * i, KDMsgWidthMin, KinkyDungeonActionMessageColor, KDTextGray1, KDMSGFontSize, undefined, zLevel);
				ignoreMSG.push(KinkyDungeonActionMessage);
				i++;
			}
			if (KinkyDungeonTextMessageTime > 0 && KinkyDungeonTextMessageNoPush) {
				DrawTextFitKD(KinkyDungeonTextMessage, KDMsgX + KDMsgWidth/2, 15 + spacing * i, KDMsgWidthMin, KinkyDungeonTextMessageColor, KDTextGray1, KDMSGFontSize, undefined, zLevel);
...
function KDTex(Image, Nearest) {
	...
	if (erroredTexStop[KDModFiles[Image] || Image] > MAX_TRIES_ASSET || CommonTime() < erroredTex[KDModFiles[Image] || Image]) return null;
	try {
		PIXI.Assets.add({
		...
		if (tex) {
			...
			erroredTex[KDModFiles[Image] || Image] = CommonTime() + 250;
			erroredTexStop[KDModFiles[Image] || Image] = (erroredTexStop[KDModFiles[Image] || Image] || 0) + 1;
		}
		return tex;
	} catch (e) {
