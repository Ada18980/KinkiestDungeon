...
function KDDrawContextMenu(draw: boolean, mouseX: number, mouseY: number,
	...
	if (draw && options.length > 0) {
		...
		let offsets = KDGetBoxShiftOffset(KDContextX-bpad, KDContextY - bpad, bwidth + 2*bpad, options.length * (bheight + bpad))
		...
		for (let o of options) {
			...
			optionImages[o] ? KinkyDungeonRootDirectory + "UI/ContextMenu/" + optionImages[o] + ".png" : "",
			undefined,
			undefined, optionGrey[o], optionGrey[o] ? KDBaseBlack : undefined, 16,
	...
	if (KinkyDungeonState == "Mods") {
		...
		for (let i = 0; i < classCount; i++) {
			...
			buttonsstart + (buttonspad + buttonswidth) * X, 190 + Y*(buttonsheight + buttonsypad), buttonswidth, buttonsheight, TextGet("KinkyDungeonClassMode" + Object.keys(KDClassStart)[i]),
				((!KDClassReqs[Object.keys(KDClassStart)[i]]) || KDClassReqs[Object.keys(KDClassStart)[i]]()) ?
				(KinkyDungeonClassMode == Object.keys(KDClassStart)[i] ? KDBaseWhite : "#888888")
				: KDBaseRed, "", undefined, undefined, true, KDButtonColor);
			if (MouseIn(buttonsstart + (buttonspad + buttonswidth) * X, 190 + Y*(buttonsheight + buttonsypad), buttonswidth, buttonsheight)) {
				DrawTextFitKD(TextGet("KinkyDungeonClassModeDesc" + Object.keys(KDClassStart)[i]), 1250, 120, 1000, KDBaseWhite, KDTextGray0);
			}
		}
	...
	*/
		...
		for (let slt of offsets) {
			...
			400, 50, (loadedsaveNames[slot-1] ? loadedsaveNames[slot - 1] : "")
				|| (TextGet("KDEmpty")),
			(danger && (slot == KDSaveSlot)) ? KDBaseRed : KDBaseWhite, "", undefined, undefined,
			true, KDButtonColor);
		}
			...
			if (!KDContextMenu) {
				if (KDGameData.SleepTurns > 0) {
					...
					if (CommonTime() > KinkyDungeonSleepTime) {
						...
						KDGameData.BalancePause = true;
						KDSendInput("tick", {delta: 1, NoUpdate: false, NoMsgTick: true}, false, true);
						KinkyDungeonSleepTime = CommonTime() + KinkyDungeonStunTime * (0.35 + KDAnimSpeed * 0.65);
					}
		...
		// Draw temp start screen
		DrawTextKD(TextGet("EndWin"), 1250, 400, KDBaseWhite, KDTextGray2);
		DrawTextKD(TextGet("EndWin2"), 1250, 500, KDBaseWhite, KDTextGray2);

		DrawButtonVis(1075, 650, 350, 64, TextGet("KinkyDungeonNewGamePlus"), KDBaseWhite, "");
		DrawButtonVis(1075, 750, 350, 64, TextGet("KinkyDungeonMenu"), KDBaseWhite, "");
	...
	if (!KDLoadCloudGames) {
		for (let i = 1; i < 5; i++) {
			...
			DrawButtonKDEx(TextGet("KDSaveSlotButton") + num, () => {
				...
				return true;
			}, true, CombarXX + 100, YY, 300, 64, TextGet("KDSaveSlotButton") + i, KDBaseWhite, "");
