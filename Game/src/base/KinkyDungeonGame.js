		...
		if (!testPlacement) {
			...
			if (!altType || altType.shrines) {
				let quests = KinkyDungeonPlaceShrines(chestlist, shrinelist, shrinechance, shrineTypes, shrinecount,
					...
					Floor, width, height, !altType || (altType.makeMain && !altType.noQuests), (!altType || altType.heart) && KDGameData.CollectedHearts/2 < (MiniGameKinkyDungeonLevel + KinkyDungeonMaxLevel*KinkyDungeonNewGame));
				if (
				//(
